...
def _generator_features_extractor(file_path):
    return get_wav_feature(file_path, mfccd=True, mfccdd=True, remove_silence=True, silence_percentile=25)
...
def load_full_dataset(list_of_files):
    result = []
...
def get_entry_language_label_gmm(gmm_models_by_language, entry):
    max_proba = 0.0
    best_language = None
    entry_reshaped = entry.reshape(1, -1)
    for language, l_models in gmm_models_by_language.iteritems():
        for model in l_models:
            cur_model_proba = model.predict_proba(entry_reshaped)[0]
            cur_language_proba = max(cur_model_proba)
            if cur_language_proba > max_proba:
                max_proba = cur_language_proba
                best_language = language
    return max_proba, best_language


def get_file_features_gmm_language_label(gmm_models_by_language, objects):
    max_proba = 0.0
    best_language = None
    
    for language, l_models in gmm_models_by_language.iteritems():
        for model in l_models:
            objects_probas = model.predict_proba(objects)
            values = []
            for a in objects_probas:
                values.append(max(a))
            cur_language_proba = np.mean(values)  # np.amax(objects_probas)
            if cur_language_proba > max_proba:
                max_proba = cur_language_proba
                best_language = language
...
def get_one_gmm():
    return mixture.GaussianMixture(n_components=128, covariance_type='diag', reg_covar=1e-3, max_iter=100, verbose=2)
...
if __name__ == "__main__":
    ...
    if _IS_TRAIN:
        ...
        print 'Loading list of files for', language
        ...
        full_dataset = load_full_dataset(train_files)
        ...
        print 'Saving gmm'
        save_one_gmm(gmm, '/home/kolegor/gmm_{}.cpickle'.format(language))
        print 'Done', language
    else:
        models_by_language = defaultdict(list)
        models_dir = '/home/kolegor/Study/Master/experiments/Gmm___256comp__50maxiter__400000lim__by5000files/'
        for file_name in os.listdir(models_dir):
            if not file_name.startswith('gmm'):
                continue
            tokens = file_name.split('_')
            language = tokens[1]

            models_by_language[language].append(load_one_gmm(os.path.join(models_dir, file_name)))
        print 'Loaded models for {} languages'.format(len(models_by_language))

        print 'Start prediction by language'
        correct_by_language = defaultdict(int)
        incorrect_by_language = defaultdict(int)
        for language, language_files_list in MAP_LANGUAGE_TO_FILES.iteritems():
            print ' * ', language
            if language not in models_by_language:
                continue

            _, test_files, _ = _split_language_files(language_files_list, 0.7, 0.3)
            print '{} test files for language {}'.format(len(test_files), language)

            print 'Loading full test files'
            for i, file_path in enumerate(test_files):
                ...
                best_proba_by_entry = 0.0
                best_language = None

                # for entry in features.T:
                #     entry_proba, entry_lang = get_entry_language_label_gmm(models_by_language, entry)
                #     if entry_proba > best_proba_by_entry:
                #         best_proba_by_entry = entry_proba
                #         best_language = entry_lang

                best_proba_by_entry, best_language = get_file_features_gmm_language_label(models_by_language, features.T)

                if best_language == language:
                    print '+', best_proba_by_entry, best_language
                    correct_by_language[language] += 1
                else:
                    print '-', best_proba_by_entry, best_language
                    incorrect_by_language[language] += 1

        print correct_by_language
        print incorrect_by_language

        for language, v in correct_by_language.iteritems():
            print language, float(v) / (v + incorrect_by_language[language])
