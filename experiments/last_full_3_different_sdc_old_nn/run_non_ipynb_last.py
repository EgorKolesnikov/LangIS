...
#     return np.vstack([mfccs, spectrogram])
    ...
    # # res = np.vstack([mfccs, sdcs_mfcc, sdcs_d, sdcs_dd])
        ...
        # N2
    mfccs = _get_wav_mfcc(values, sr, mfccd=True, mfccdd=True)
    
    sdcs_mfcc_1 = _get_sdc_features(mfccs[:13, :].T, d=1, p=3, k=7).T
    sdcs_d_1 = _get_sdc_features(mfccs[13:26, :].T, d=1, p=3, k=7).T
    sdcs_dd_1 = _get_sdc_features(mfccs[26:39, :].T, d=1, p=3, k=7).T

    sdcs_mfcc_2 = _get_sdc_features(mfccs[:13, :].T, d=3, p=1, k=5).T
    sdcs_d_2 = _get_sdc_features(mfccs[13:26, :].T, d=3, p=1, k=5).T
    sdcs_dd_2 = _get_sdc_features(mfccs[26:39, :].T, d=3, p=1, k=5).T

    sdcs_mfcc_3 = _get_sdc_features(mfccs[:13, :].T, d=1, p=1, k=7).T
    sdcs_d_3 = _get_sdc_features(mfccs[13:26, :].T, d=1, p=1, k=7).T
    sdcs_dd_3 = _get_sdc_features(mfccs[26:39, :].T, d=1, p=1, k=7).T
    
    res = np.vstack([mfccs, sdcs_mfcc_1, sdcs_d_1, sdcs_dd_1, sdcs_mfcc_2, sdcs_d_2, sdcs_dd_2, sdcs_mfcc_3, sdcs_d_3, sdcs_dd_3])
...
def _generator_features_extractor(file_path):
    return get_wav_feature(file_path, remove_silence=True)
...
model = load_model('/home/kolegor/Study/Master/experiments/last_full_3_different_sdc_old_nn/model.last_full_3_different_sdc_old_nn.keras')
...
if False:
    # 1727546/1727545 [==============================] - 44864s 26ms/step - loss: 0.4599 - acc: 0.8628 - val_loss: 3.5733 - val_acc: 0.4346
    # Epoch 2/3
    # 1727546/1727545 [==============================] - 44998s 26ms/step - loss: 0.4399 - acc: 0.8643 - val_loss: 3.8898 - val_acc: 0.3813
    # Epoch 3/3
    # 1727546/1727545 [==============================] - 44600s 26ms/step - loss: 0.4207 - acc: 0.8705 - val_loss: 3.5227 - val_acc: 0.4316
    epochs = 3
    ...
    model.save('/home/kolegor/Study/Master/experiments/last_full_3_different_sdc_old_nn/model.last_full_3_different_sdc_old_nn.keras')
else:
    ...
    # 1406381 2205643 0.637628573618
